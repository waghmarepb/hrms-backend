...
class Income
{
    ...
    public function findById($id)
    {
        $sql = "SELECT 
                ...
                FROM {$this->table} t
                LEFT JOIN acc_coa c ON t.COAID = c.HeadCode
                WHERE t.{$this->primaryKey} = ? AND t.Vtype = 'Income'
                LIMIT 1";
    ...
    public function delete($id)
    {
        return $this->db->delete(
            $this->table,
            "WHERE {$this->primaryKey} = ? AND Vtype = 'Income'",
            [$id]
        );
    }
    ...
    public function generateVoucherNo()
    {
        $result = $this->db->selectOne(
            "SELECT MAX(CAST(SUBSTRING(VNo, 2) AS UNSIGNED)) as max_no 
             FROM {$this->table} 
             WHERE Vtype = 'Income' AND VNo LIKE 'I%'"
        );
        
        $nextNo = ($result['max_no'] ?? 0) + 1;
        return 'I' . str_pad($nextNo, 6, '0', STR_PAD_LEFT);
    }
}
